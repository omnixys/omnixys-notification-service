<!-- repository: services/notification | kind: SERVICE | stack: nestjs -->

# notification — Skill: Service Development

> Workflow for notification (services/notification). Execute this workflow before, during, and
...
- Description: Omnixys Notification Service – notifications, modules, admin, analytics.
- Architecture: src/adapter, admin, analytics, config, core, handlers, health, modules, prisma, security, utils
...
- Tests: node --test __tests__/unit/*.test.mjs and __tests__/integration/*.test.mjs; Jest e2e
...
- Identify the affected bounded context within `src/adapter, admin, analytics, config, core, handlers, health, modules, prisma, security, utils`.
...
- Keep tenant isolation intact (`Tenant-aware notifications; delivery/observability relevant.`).
